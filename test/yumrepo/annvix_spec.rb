...
describe 'Repo::Attributes::Default' do
  ...
  before do
    ...
    @node.from_file(File.join(File.dirname(__FILE__), %w{.. .. attributes annvix.rb}))
  end
  ...
  describe "for centos platform version 6.2" do
    ...
    it "sets the annvix.url to" do
      @node[attr_ns]['annvix']['url'].must_equal "http://repo.annvix.org/media/EL6/$basearch"
    end
  end
end
